...
// Remove internal links pointing to non-existent pages (prevents 404s from AI hallucinated links)
...
// Use Gemini to create a maximally safe image prompt that avoids content policy violations
async function generateSafePrompt(text, categorySlug) {
  const categoryFallbacks = {
    'gradinarit-unelte': 'garden tools arranged on a wooden table surrounded by green plants',
    'mobilier-gradina': 'outdoor patio furniture on a sunny terrace with potted plants',
    'electrocasnice-casa': 'modern kitchen appliances on a clean white countertop',
    'decoratiuni-casa': 'decorative objects and candles on a modern living room shelf',
    'curatenie-intretinere': 'cleaning supplies and bottles arranged on a bright bathroom shelf',
  };
  ...
  return categoryFallbacks[categorySlug] || 'household objects and products arranged on a clean modern surface';
}

async function generateImage(titleRo, slug, categorySlug) {
  const categoryPrompts = {
    'gradinarit-unelte': 'in a beautiful garden setting, natural daylight, lush greenery, clean aesthetic',
    'mobilier-gradina': 'on a sunny terrace or garden patio, natural daylight, lush greenery, outdoor living',
    'electrocasnice-casa': 'in a modern kitchen or home interior, clean contemporary setting, soft natural lighting',
    'decoratiuni-casa': 'in a beautifully decorated modern living room, warm ambient lighting, cozy interior design',
    'curatenie-intretinere': 'in a bright clean modern home interior, fresh and organized space, soft natural lighting',
  };
...
const prompt = `Scrii articole pe un site romanesc despre produse pentru casa, gradina si amenajari interioare. Stilul e informativ dar directionat spre conversie - ajuti cititorul sa ia o decizie de cumparare. NU esti un AI.
...
- Translatezi specs in beneficii practice: "puterea de 2000W inseamna ca incalzeste o camera de 25 mp in 15 minute"
...
- Exemplu bun: "Consuma mult curent? Nu, modelul X are clasa energetica A++ si un consum mediu de 0.8 kWh, ceea ce inseamna costuri sub 50 lei pe luna."
  ...
  "items": [
    {
      ...
      "tag": "Best Buy 2026",
      "specs": {
        "tip": "ex: convector electric / aer conditionat split",
        "putere": "ex: 2000W / 12000 BTU",
        "suprafata": "ex: acoperire 25-30 mp",
        "functii": "ex: termostat digital, timer, telecomanda",
        "material": "ex: otel inoxidabil, plastic ABS"
      },
    ...
    "rows": [
      {
        ...
        "putere": "putere/capacitate",
        "suprafata": "acoperire",
        "functii": "functii principale",
        "potrivitPentru": "scurt, 3-5 cuvinte"
      }
    ]
  },
  "guide": {
    "title": "Titlu ca intrebare (ex: Cum alegi cel mai bun produs pentru casa ta?)",
...
${interlinkList}` : ''}`;
  ...
  throw new Error('Failed to generate content after 3 retries');
}
...
<Layout
  title="${escapeForHtml(title)} - CentroCasa"
  ...
  <article class="article-container">
    ...
    <header class="article-header">
      <div class="article-meta">
        <a href="/${categorySlug}/" class="article-category">${category}</a>
        <span class="article-date">${pubDateDisplay}</span>
      </div>
      <h1 class="article-title">${title}</h1>
    </header>

    <!-- Featured Image -->
    ${imagePath ? `<img src="${imagePath}" alt="${escapeForHtml(title)}" class="article-image" width="800" height="600" loading="eager">` : ''}
...
async function main() {
  console.log('\n========================================');
  console.log('CentroCasa.ro - Article Generator');
  console.log('========================================\n');
