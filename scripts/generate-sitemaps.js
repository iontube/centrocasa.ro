...
/**
 * Generate WordPress-style sitemaps for centrocasa.ro
 * Creates:
...
const SITE_URL = 'https://centrocasa.ro';
...
// Get the most recent article date (prefers modifiedDate over date)
...
// Generate sitemap index (only articles and categories)
...
function generatePostSitemap() {
  ...
  for (const article of articles) {
	...
	<url>
		...
		<lastmod>${(article.modifiedDate || article.date) ? new Date(article.modifiedDate || article.date).toISOString().replace(/\.\d{3}Z$/, '+00:00') : now}</lastmod>${hasImage ? `
		<image:image>
...
// Generate XSL stylesheet for pretty sitemap display
...
<head>
  <title>XML Sitemap - CentroCasa.ro</title>
  ...
  <style type="text/css">
    ...
    h1 { color: #166534; border-bottom: 3px solid #22c55e; padding-bottom: 10px; }
    ...
    .count { background: #22c55e; color: white; padding: 2px 8px; border-radius: 12px; font-size: 12px; }
  </style>
</head>
<body>
  <h1>XML Sitemap</h1>
