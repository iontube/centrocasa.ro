...
const NO_IMG=process.argv.includes('--no-img'); const GROUP='masina-spalat-rufe';
...
const PICK=['DLGFKBYBM','DG88TPYBM','DGJ5KY2BM','D1C6P3YBM','DCGFKBYBM','DCLH27YBM','DJX282MBM','D7MHTFYBM','DZ0NQCYBM','DWH95PYBM','D5YSGRYBM','DLFDFMYBM'];
...
list.forEach((r,i)=>console.log((i+1)+'. rev='+r.reviews+' rat='+r.rating+' '+r.price+'lei | '+r.name.replace(/\s+/g,' ').slice(0,50)));
...
console.log('grup '+GROUP+(ex?' ACTUALIZAT':' ADAUGAT')+' | grupuri:',Object.keys(db).length,'| tuns-iarba/frigidere neatinse');
